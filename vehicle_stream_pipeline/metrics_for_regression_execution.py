...
regression_metrics = fa.getRegressionMetrics(
    df_simulated, df_edges, 10000, only_main_routes=True
)
regression_metrics_main_routes = fa.getRegressionMetrics(
    rides_main_routes, df_edges, 100, 1000, only_main_routes=False
)

# save regression metrics as csv
regression_metrics.to_csv(f"{repo}/data/regression/graph_metrics_test.csv")
regression_metrics_main_routes.to_csv(
    f"{repo}/data/regression/graph_metrics_main_routes_test.csv"
)
