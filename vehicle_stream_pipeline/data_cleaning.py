...
class Cleaning:
    ...
    def clean_duplicates(df):
        duplicate_ids = df[df.duplicated(subset=["id"]) & (df["id"].isna() == False)][
            "id"
        ]
        ...
        for col, col_type in col_type_dict.items():
            if col_type == "timestamp":
                ...
                df_inconsistencies = pd.concat(
                    [df_inconsistencies, df_inconsistencies_temp],
                    axis=0,
                    ignore_index=True,
                )
                df = df[
    ...
    def clean_addresses(df, df_stops):
        ...
        for index, row in addresses.iterrows():
            addresses.at[index, "pickup_id"] = Cleaning.get_stop_id(
                row["pickup_address"], df_stops
            )
            addresses.at[index, "dropoff_id"] = Cleaning.get_stop_id(
        ...
        addresses.drop(
            columns=["pickup_address", "dropoff_address"], axis=1, inplace=True
        )
        return addresses
            ...
            np.where(
                ...
                np.where(
                    (
                        ...
                        + pd.Timedelta(seconds=avg_boarding_time)
                        < df["dropoff_at"]
                    )
    ...
    def clean_time_periods(df):
        ...
        df["waiting_time"] = df.apply(
            ...
            if (row["vehicle_arrived_at"] == row["vehicle_arrived_at"])
            and (
                row["earliest_pickup_expectation"] == row["earliest_pickup_expectation"]
            )
            else np.NaN,
            axis=1,
        )
        ...
        df["shortest_ridetime"] = df.apply(
            lambda row: (
                pd.to_timedelta(row["shortest_ridetime"])
                .round(freq="s")
                .total_seconds()
            )
    ...
    def data_cleaning(df, df_stops):
        ...
        df, df_inconsistencies = Cleaning.check_format(df, columns)
        ...
        print("clean id")
        df["id"] = Cleaning.clean_ride_id(df)

        print("clean distance")
        df = Cleaning.clean_distance(
            df
        )  # Hier mit gesamten df, da Zeilen gelöscht werden

        print("clean addresses")
        df[["pickup_address", "dropoff_address"]] = Cleaning.clean_addresses(
            df, df_stops
        )

        print("clean free_rides")
        df["free_ride"] = Cleaning.clean_free_ride(df)

        print("clean created_at")
        df["created_at"] = Cleaning.clean_created_at(df)

        print("clean scheduled_to")
        df["scheduled_to"] = Cleaning.clean_scheduled_to(df)

        print("clean dispatched_at")
        df["dispatched_at"] = Cleaning.clean_dispatched_at(df)

        print("clean vehicle_arrived_at")
        df["vehicle_arrived_at"] = Cleaning.clean_vehicle_arrived_at(df)

        print("clean arriving_push")
        df["arriving_push"] = Cleaning.clean_arriving_push(df)

        print("clean earliest_pickup_expectation")
        df["earliest_pickup_expectation"] = Cleaning.clean_earlierst_pickup_expectation(
            df
        )

        print("clean pickup_at")
        df["pickup_at"] = Cleaning.clean_pickup_at(df)

        print("clean pickup_eta")
        df["pickup_eta"] = Cleaning.clean_pickup_eta(df)

        print("clean pickup_first_eta")
        df["pickup_first_eta"] = Cleaning.clean_pickup_first_eta(df)

        print("clean dropoff_at")
        df["dropoff_at"] = Cleaning.clean_dropoff_at(df)

        print("clean dropoff_eta")
        df["dropoff_eta"] = Cleaning.clean_dropoff_eta(df)

        print("clean dropoff_first_eta")
        df["dropoff_first_eta"] = Cleaning.clean_dropoff_first_eta(df)

        print("clean time periods")
        df = Cleaning.clean_time_periods(df)

        print("clean rating")
        df["rating"] = Cleaning.clean_rating(df)
        return df
...
if __name__ == "__main__":
    ...
    df = Cleaning.clean_duplicates(df)

    df = Cleaning.data_cleaning(df, df_stops)
